...
void *sort(void *args){
	...
	masterarraycount=m;
	masterarray=realloc(masterarray,masterarraysize);
...
int main(int argc, char **argv)
{
	...
	for (;i<argc;i++)
	{
		if(strcmp(argv[i], "-o")==0)
		{
			...
			if(i%2==1)
			{
				printf("ERROR: Improper use  or incorrect number of flags!\n");
				return-1;
			}
			hasoutput=1;
			outputdir=argv[i+1];
    ...
    char path[4096]; // THIS IS THE STARTING PATH, NEED TO ADD IN INPUT CHECKS FOR THE RIGHT PATH :)
	...
	free(masterarray);
    printf("\nEverything is freed, everything waited on, good work!\n");
